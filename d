...
-- Define a safe way to make HTTP requests
local function safeHttpRequest(url)
    local success, response = pcall(function()
        return game:HttpGet(url)  -- Use HttpGet for compatibility with Solara
    end)
...
-- Load and execute the external library
local libraryUrl = "https://raw.githubusercontent.com/cypherdh/Script-Library/main/InstanceProtect"
local libraryCode = safeHttpRequest(libraryUrl)

if libraryCode then
    local success, library = pcall(loadstring(libraryCode))
    if not success then
        warn("Failed to load library:", library)
    end
else
    warn("Library code could not be retrieved.")
end
...
-- Additional code using `library`, `UIS`, and `TS` goes here
